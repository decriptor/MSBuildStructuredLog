...
{
	public class BuildControl : VBox
	{
		...
		ArchiveFileResolver archiveFile => sourceFileResolver.ArchiveFile;
		PreprocessedFileManager preprocessedFileManager;

		MenuItem copyItem;
		MenuItem copySubtreeItem;
		MenuItem sortChildrenItem;
		MenuItem copyNameItem;
		MenuItem copyValueItem;
		MenuItem viewItem;
		MenuItem preprocessItem;
		MenuItem hideItem;
		MenuItem copyAllItem;
		//ContextMenu sharedTreeContextMenu;

		TreeView ActiveTreeView;
		TreeStore ActiveTreeStore;
		...
		public BuildControl(Build build, string logFilePath)
		{
			...
			searchLogControl = new SearchAndResultsControl();
			...
			ActiveTreeStore = new TreeStore(iconCol, nameCol, baseNode);
			ActiveTreeView = new TreeView (ActiveTreeStore) {
				HeadersVisible = false,
				ExpandHorizontal = true
			};
			var buildNodeTextView = new TextCellView {
				MarkupField = nameCol,
			};
			var buildNodeColumn = new ListViewColumn("") {
				Expands = true
			};
			buildNodeColumn.Views.Add(new ImageCellView(iconCol));
			buildNodeColumn.Views.Add(buildNodeTextView, true);
			ActiveTreeView.Columns.Add(buildNodeColumn);
...
//Right-clicking a project node may show the 'Preprocess' option if the version of MSBuild was at least 15.3.";
			...
			ActiveTreeViewBox.PackStart(ActiveTreeView, true);

			treeAndSourceBox.Panel1.Content = ActiveTreeViewBox;
			treeAndSourceBox.Panel1.Resize = true;

			PackStart(boxes, true);
		...
		TreePosition root;
		void RefreshTree(Build build)
		{
			root = ActiveTreeStore.AddNode(null).SetValue(nameCol, build.Name).SetValue(baseNode, build).CurrentPosition;
			AddNodes(root, build.Children);
		}

		void AddNodes(TreePosition pos, IList<object> nodes)
		{
			foreach (var node in nodes) {
				switch (node) {
					case Property property:
						var propertyText = $"<span color='{Colors.DodgerBlue.ToHexString()}'>{property.Name} = </span> {property.Value}";
						var propertyRoot = ActiveTreeStore.AddNode(pos).SetValue(nameCol, propertyText).SetValue(baseNode, property).CurrentPosition;
						if (property.IsExpanded)
							ActiveTreeView.ExpandRow(propertyRoot, false);
						break;
					case Metadata metadata:
						var metadataText = $"<span color='{Colors.MediumAquamarine.ToHexString()}'>{metadata.ToString()}</span>";
						var metadataRoot = ActiveTreeStore.AddNode(pos).SetValue(nameCol, metadataText).SetValue(baseNode, metadata).CurrentPosition;
						if (metadata.IsExpanded)
							ActiveTreeView.ExpandRow(metadataRoot, false);
						break;
					case Folder folder:
						var folderText = $"<span color='{Colors.Goldenrod.ToHexString()}'>{folder.ToString()}</span>";
						var folderRoot = ActiveTreeStore.AddNode(pos).SetValue(nameCol, folderText).SetValue(baseNode, folder).CurrentPosition;
						AddNodes(folderRoot, folder.Children);
						if (folder.IsExpanded)
							ActiveTreeView.ExpandRow(folderRoot, false);
						break;
					case Project project:
						var projectRoot = ActiveTreeStore.AddNode(pos).SetValue(nameCol, project.ToString()).SetValue(baseNode, project).CurrentPosition;
						AddNodes(projectRoot, project.Children);
						if (project.IsExpanded)
							ActiveTreeView.ExpandRow(projectRoot, false);
						break;
					case Target target:
						var targetText = $"<span color='{Colors.MediumPurple.ToHexString()}'>Target </span> {target.Name}";
						var targetRoot = ActiveTreeStore.AddNode(pos).SetValue(nameCol, targetText).SetValue(baseNode, target).CurrentPosition;
						AddNodes(targetRoot, target.Children);
						if (target.IsExpanded)
							ActiveTreeView.ExpandRow(targetRoot, false);
						break;
					case Message message:
						var messageText = $"<span color='{Colors.LightGray.ToHexString()}'>{message.ToString()}</span>";
						var messageRoot = ActiveTreeStore.AddNode(pos).SetValue(nameCol, messageText).SetValue(baseNode, message).CurrentPosition;
						AddNodes(messageRoot, message.Children);
						if (message.IsExpanded)
							ActiveTreeView.ExpandRow(messageRoot, false);
						break;
					case Task task:
						var taskText = $"<span color='{Colors.DeepSkyBlue.ToHexString()}'>Task </span> {task.Name}";
						var taskRoot = ActiveTreeStore.AddNode(pos).SetValue(nameCol, taskText).SetValue(baseNode, task).CurrentPosition;
						AddNodes(taskRoot, task.Children);
						if (task.IsExpanded)
							ActiveTreeView.ExpandRow(taskRoot, false);
						break;
					case Item item:
						var itemText = $"<span color='{Colors.MediumAquamarine.ToHexString()}'>{item.ToString()}</span>";
						var itemRoot = ActiveTreeStore.AddNode(pos).SetValue(nameCol, itemText).SetValue(baseNode, item).CurrentPosition;
						AddNodes(itemRoot, item.Children);
						if (item.IsExpanded)
							ActiveTreeView.ExpandRow(itemRoot, false);
						break;
					case AddItem addItem:
						var addItemRoot = ActiveTreeStore.AddNode(pos).SetValue(nameCol, addItem.ToString()).SetValue(baseNode, addItem).CurrentPosition;
						AddNodes(addItemRoot, addItem.Children);
						if (addItem.IsExpanded)
							ActiveTreeView.ExpandRow(addItemRoot, false);
						break;
					case RemoveItem removeItem:
						var removeItemRoot = ActiveTreeStore.AddNode(pos).SetValue(nameCol, removeItem.ToString()).SetValue(baseNode, removeItem).CurrentPosition;
						AddNodes(removeItemRoot, removeItem.Children);
						if (removeItem.IsExpanded)
							ActiveTreeView.ExpandRow(removeItemRoot, false);
						break;
					case Parameter parameter:
						var parameterText = $"<span color='{Colors.DodgerBlue.ToHexString()}'>{parameter.ToString()}</span>";
						var parameterRoot = ActiveTreeStore.AddNode(pos).SetValue(nameCol, parameterText).SetValue(baseNode, parameter).CurrentPosition;
						AddNodes(parameterRoot, parameter.Children);
						if (parameter.IsExpanded)
							ActiveTreeView.ExpandRow(parameterRoot, false);
						break;
					case Warning warning:
						var warningText = $"<span color='{Colors.Yellow.ToHexString()}'>{warning.ToString()} </span>";
						var warningRoot = ActiveTreeStore.AddNode(pos).SetValue(nameCol, warningText).SetValue(baseNode, warning).CurrentPosition;
						AddNodes(warningRoot, warning.Children);
						if (warning.IsExpanded)
							ActiveTreeView.ExpandRow(warningRoot, false);
						break;
					case Error error:
						var errorText = $"<span color='{Colors.LightCoral.ToHexString()}'>{error.ToString()} </span>";
						var errorRoot = ActiveTreeStore.AddNode(pos).SetValue(nameCol, errorText).SetValue(baseNode, error).CurrentPosition;
						AddNodes(errorRoot, error.Children);
						if (error.IsExpanded)
							ActiveTreeView.ExpandRow(errorRoot, false);
						break;
					default:
						Console.WriteLine($"Missing Node type {node.GetType().ToString()}");
						break;
				}
			}
		}
...
//");
		...
		private IEnumerable<object> BuildFindResults(object resultsObject)
		{
		...
		private IEnumerable<object> BuildResultTree(object resultsObject)
		{
